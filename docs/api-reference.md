# API Reference — fhir-definition v0.4.0
...
```typescript
class PackageLoader {
  loadPackage(pkg: DefinitionPackage, options?: { resourceTypes?: FhirDefinitionResourceType[] }): PackageLoadResult;
}
```
...
```typescript
function loadFromDirectory(dirPath: string, options?: LoadDirectoryOptions): DefinitionRegistry;
```
...
```typescript
function loadDefinitionPackages(rootPath: string, options?: LoadPackagesOptions): LoadPackagesOutput;
```
...
console.log('Loaded packages:', result.loadedPackages.map(p => p.name));
```

### Pattern 3: Runtime Integration

```typescript
import { loadDefinitionPackages, type DefinitionProvider } from 'fhir-definition';
...
```typescript
function isStructureDefinition(r: FhirDefinitionResource): r is StructureDefinition {
